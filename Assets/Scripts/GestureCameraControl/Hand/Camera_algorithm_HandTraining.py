...
totalTimer = 3
totalFrames = 5
continuous_capture = False  # Flag for continuous capture mode
...
def train_hand_pose(output_filename):
    ...
    while cam.isOpened():
        ...
        # Toggle continuous capture when 'a' is pressed
        if key == ord('a'):
            continuous_capture = not continuous_capture
            if continuous_capture:
                print("Continuous capture started. Press 'a' again to stop.")
            else:
                print("Continuous capture stopped.")

        # Continuous capture mode
        if continuous_capture and hands_detected.multi_hand_landmarks:
            for hand_landmarks in hands_detected.multi_hand_landmarks:
                keypoints = np.array([[lm.x, lm.y, lm.z] for lm in hand_landmarks.landmark])
                hand_samples.append(keypoints)
                print("Captured a hand pose in continuous mode.")
            ...
            for j in range(totalFrames):  # Capture 5 frames
                ...
                if hands_detected.multi_hand_landmarks:
                    for hand_landmarks in hands_detected.multi_hand_landmarks:
                        keypoints = np.array([[lm.x, lm.y, lm.z] for lm in hand_landmarks.landmark])
                        hand_samples.append(keypoints)
                        print(f"Captured hand pose {j + 1} of {totalFrames}.")

                # Wait 1 second between captures
                time.sleep(1)

        # Save the average pose when 's' is pressed
        if key == ord('s') and hand_samples:
            reference_pose = calculate_average_pose(hand_samples)
            np.save(output_filepath, reference_pose)
            print(f"Hand pose saved to {output_filepath}!")
            break
...
#     """Normalize landmarks based on the width and height of the frame."""
#     normalized = np.array([
#         [lm.x * frame_width, lm.y * frame_height, lm.z * frame_width] for lm in landmarks
#     ])
#     return normalized
...
#                 )
#                 hand_samples.append(normalized_landmarks)
#                 print("Captured a hand pose in continuous mode.")
