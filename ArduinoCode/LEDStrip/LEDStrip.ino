...
// Button
constexpr int buttonPin = A1;     // Compile-time constant for the button pin
...
void serialMode(){
  if(Serial.available() > 0)
  {
    ...
    if(token.equals("load")) //load
    {
      Serial.println("Load strip");
      STATUS = 4;
    }

    else if(token.equals("blue")) //load
    {
      Serial.println("Flash Blue strip");
      STATUS = 4;
    }

    else if(token.equals("clear")) // clear
    {
      Serial.println("Clear strip");
      STATUS = 1;
    }

    else if(token.equals("green")) // good
    {
      Serial.println("Flash Green light");
      STATUS = 2;
    }

    else if(token.equals("red")) // bad
    {
      Serial.println("Flash Red light");
      STATUS = 3;
    }
    else if(token.equals("random")) // random
    {
...
void lightMode(){
  switch (STATUS) {
    ...
    case 2: { // Gravity up sign
        // colorWipe(strip.Color(  0, 255,   0), 50); // Green
        // theaterChase(strip.Color(  0,   200, 0), 20); // Green, half brightness
    ...
    case 3 : { // Gravity Down sign
        // colorWipe(strip.Color(  255, 0,   0), 50); // Red
        // theaterChase(strip.Color(  200,   0, 0), 20); // Red, half brightness
    ...
    case 4 : { // Blue sign
        // colorWipe(strip.Color(  0, 0,   255), 50); // Blue
        theaterChase(strip.Color(  0,   0, 200), 20); // Blue, half brightness
        STATUS = 1;
      }
      break;
